...
<div class="pro-m">
    ...
    <div class="pro-m-t1">
    	<a href="<?php echo $this->url("Product","IRobotLAPic");?>" class="c4 hover"></a>
        <a href="<?php echo $this->url("Product","IRobotSCPic");?>" class="c5"></a>
...
<?php } ?>
    <div class="d5">
        ...
        <img id="img_b_<?php echo $k+1;?>" src="<?php echo $this->url(($row['filename']&&$row['filename']!='')?$row['filename']:"img/la-".($k+1).".png");?>" style="height:323px;<?php echo $k==0?'':'display:none;';?>" />
        <?php } ?>
    </div>
    <div class="d4">
        <?php foreach ($pic_list as $k => $row) {?>
        <a href="javascript:void(0);"><img id="img_s_<?php echo $k+1;?>" src="<?php echo $this->url(($row['filename']&&$row['filename']!='')?$row['filename']:"img/la-".($k+1).".png");?>" style="height:95px;<?php echo $k==3?'margin-left:-32px;':'';?>" /></a>
        <?php } ?>
    </div>
    
    <div class="pro-m-f fix">
        <a href="<?php echo $this->url("Product", "IRobotLA");?>">介绍</a>
        <a class="hover">应用</a>
